...
class IcloudAccount:
    ...
    def setup(self) -> None:
        ...
        try:
            ...
            # The session can expire between logging in and the first fetch.
            # Ask for reauth, but still raise: the fetch timer is only armed
            # at the end of update_devices(), so returning here would leave an
            # entry that is loaded and never polls again.
