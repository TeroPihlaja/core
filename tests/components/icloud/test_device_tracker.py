...
def location(*, is_old: bool, age: timedelta) -> dict:
    ...
    fixed_at = datetime.now(tz=UTC) - age
    return {
        "latitude": 1.0,
        "longitude": 2.0,
...
async def test_recent_cached_location_kept(
    ...
    iCloud flags a fix as cached whenever the device is merely asleep, so the
    flag alone must not be enough to discard a location.
    """
    _, status = locating_service
    await setup_account(hass)
