...
@pytest.mark.usefixtures("service_2fa")
async def test_setup_2fa_does_not_report_bad_password(
    hass: HomeAssistant, caplog: pytest.LogCaptureFixture
) -> None:
    """Test that a 2FA challenge is not reported as a wrong password."""
    config_entry = MockConfigEntry(
        domain=DOMAIN, data=MOCK_CONFIG, entry_id="test", unique_id=USERNAME
    )
    config_entry.add_to_hass(hass)
    ...
    # The reauth flow reuses this session to validate the code, so it has to
...
async def test_auth_required_on_first_fetch_is_retried(
    hass: HomeAssistant, service_2fa
) -> None:
